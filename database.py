...
def init(config):
    ...
    CREATE TABLE IF NOT EXISTS teams(
      id INTEGER PRIMARY KEY,name TEXT NOT NULL,registration_code TEXT UNIQUE,
      vk_id INTEGER UNIQUE,captain TEXT,word_i INTEGER DEFAULT 0,
      word_draft TEXT DEFAULT '[]',sentence_draft TEXT DEFAULT '[]',
    ...
    for tid,x in config.items():
        c.execute('''INSERT OR IGNORE INTO teams(id,name,registration_code)
                     VALUES(?,?,?)''',(tid,x['name'],x['registration_code']))
        c.execute('''UPDATE teams SET name=?,registration_code=? WHERE id=?''',
                  (x['name'],x['registration_code'],tid))
    c.commit(); c.close()
...
def get_(k,d=None):
    c=conn(); r=c.execute('SELECT v FROM settings WHERE k=?',(k,)).fetchone(); c.close(); return r['v'] if r else d
...
def by_reg(code):
    c=conn(); r=c.execute('SELECT * FROM teams WHERE registration_code=?',(code,)).fetchone(); c.close(); return r
...
def reset(tid):
    c=conn(); c.execute("UPDATE teams SET word_i=0,word_draft='[]',sentence_draft='[]',finished=0,finished_at=NULL WHERE id=?",(tid,)); c.execute('DELETE FROM awards WHERE team_id=?',(tid,)); c.execute('DELETE FROM words WHERE team_id=?',(tid,)); c.commit(); c.close()
